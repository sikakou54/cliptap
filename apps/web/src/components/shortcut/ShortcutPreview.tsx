 ...
 * @see apps/mobile/src/components/shortcut/ShortcutValuePreview.tsx - モバイル版
 */
...
export function ShortcutPreview({
  ...
  const previewRows = useMemo(
    () => values.map((entry) => ({
      key: entry.key,
      name: entry.name || t('shortcut.value_name'),
      expandedValue: expandVariables(entry.value, selectedProfileId, defaultProfileId),
    })),
    [defaultProfileId, expandVariables, selectedProfileId, t, values]
  );
            ...
            return (
              ...
              >
                ...
                <span className="flex items-center">
                  <span className="truncate text-xs font-medium text-gray-400 dark:text-[#707070]">{row.name}</span>
                  {!isEmpty && (
                    <span className={`ml-1 shrink-0 ${isCopied ? 'text-emerald-500 dark:text-emerald-400' : 'text-gray-500 dark:text-[#A0A0A0]'}`}>
                      {isCopied ? (
