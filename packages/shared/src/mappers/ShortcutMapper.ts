 ...
 * isMaskedは表示だけを伏せる指定で、コピーと挿入は常にvalueをそのまま使う。
 */
const ShortcutValueQueries = {
  ...
  /* ショートカット値を新規作成 */
  INSERT: `INSERT INTO shortcut_values (id, shortcutId, value, isMasked, useCount, sortOrder, createdAt, updatedAt)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  /* ショートカット値を更新（useCountは挿入時にだけ動かすため触れない） */
  UPDATE: `UPDATE shortcut_values SET value = ?, isMasked = ?, sortOrder = ?, updatedAt = ? WHERE id = ?`,
  /* ショートカット値を削除 */
...
const toValue = (row: any): ShortcutValue => ({
  ...
  isMasked: Boolean(row.isMasked), /* 表示を伏せるか（SQLiteでは0/1で格納） */
