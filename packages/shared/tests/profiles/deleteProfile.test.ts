...
describe('ProfileService.deleteWithAutoSwitch', () => {
   ...
   *
   * @remarks
   * 紐づけを渡さなければ0件（全プロファイル向け）。
   */
  const insertShortcut = (id: string, profileIds: readonly string[]): void => {
    db?.run("INSERT INTO shortcuts VALUES (?, NULL, ?, '080', 0, 0, ?, ?)", [
      id,
      id,
      'created',
      'updated',
   ...
   * 本体ごと消すと、残るプロファイルで使っていたショートカットまで失われる。
   */
  ...
  /**
   * 他の紐づけが実在しないプロファイルを指すだけなら、削除したプロファイル専用として消す。
